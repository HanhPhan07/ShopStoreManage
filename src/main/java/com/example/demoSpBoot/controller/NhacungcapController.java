...
public class NhacungcapController {
	...
	@GetMapping("/NCCs")
	/* ---------------- GET ALL NCC ------------------------ */
	public ResponseEntity<List<nhacungcap>> findAllNCCs() {
		//return new ResponseEntity<ServiceResult>(customerService.findAll(), HttpStatus.OK);
		
		List<nhacungcap> listNCC= nhaccService.findAll();
		if(listNCC.isEmpty()) {
			return new ResponseEntity<List<nhacungcap>>(HttpStatus.NO_CONTENT);
		}
		return new ResponseEntity<List<nhacungcap>>(listNCC, HttpStatus.OK);
	}
	/* ---------------- GET NCC BY ID ------------------------ */
	@GetMapping("/NCCs/{id}")
